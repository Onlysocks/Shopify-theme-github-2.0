...
Auto-population runs only when the customer selects that tier. Inactive auto-populate tiers start empty. Switching away clears every selection in that auto-populate tier, including customer edits; returning fills it again. Clicking the already-selected tier preserves current edits. Tier 1 and tiers with auto-populate switched off retain their existing behaviour. When the current product is absent from the tier collection, auto-populate leaves the tier empty for manual selection.

## Collapsed-tier price previews

Tiers 2 and 3 each have a Show price preview when tier is collapsed toggle, off by default. When enabled, their collapsed headers show a calculation with all slots priced as the current product’s first in-stock variant, matching auto-population. The current product must be in the tier collection and have a Size option. The preview uses the tier’s existing free-pair and percentage settings, including automatic savings mode. It never changes real selections, completeness, free-slot badges, cart contents or the Add to cart button. Selecting a tier returns to actual selection pricing; collapsing it restores the preview. Missing or sold-out current products use the normal header behaviour. Tier 1 is unchanged.
